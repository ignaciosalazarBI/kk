...
def supabase_insert(table: str, payload: dict) -> tuple[bool, str]:
    try:
...
query_participar = st.query_params.get("participar", "") == "1"
default_index = 2 if query_participar else 0
menu = st.sidebar.radio("Control Pyme", menu_options, index=default_index)
st.sidebar.caption("Beta pública · Chile")

if menu == "🏠 Inicio":
    st.markdown(
        '<div class="hero"><h2 style="margin:0">Hoy en tu negocio</h2>'
        '<div style="color:#667085;margin-top:6px">Entiende en segundos cómo estás y qué deberías hacer primero.</div></div>',
        unsafe_allow_html=True,
    )
    cols = st.columns(5)
    with cols[0]:
        kpi("Ventas mes", money(KPIS["ventas"]), "Neto")
    with cols[1]:
        kpi("Margen bruto", f'{KPIS["margen_pct"]:.1%}'.replace(".", ","), money(KPIS["margen"]))
    with cols[2]:
        kpi("Caja disponible", money(KPIS["caja"]), "Atención inmediata")
    with cols[3]:
        kpi("Por cobrar", money(KPIS["por_cobrar"]), "3 documentos")
    with cols[4]:
        kpi("Costo ventas", money(KPIS["costo_ventas"]), "Mes actual")
    ...
    a, b, c = st.columns(3)
    with a:
        kpi("Cobros esperados", money(cobros_30), "Incluye vencidos")
    with b:
        kpi("Pagos esperados", money(pagos_30), "30 días")
    with c:
        kpi("Caja proyectada 30d", money(caja_30), "Si cobras/pagas según calendario")
    ...
    st.markdown(
        '<div class="cta"><h3>🚀 Buscamos las primeras Pymes Beta</h3>'
        '<p>Prueba Control Pyme, cuéntanos qué necesitas y ayúdanos a construir una herramienta pensada para dueños de negocio.</p></div>',
        unsafe_allow_html=True,
    )
    st.link_button("Quiero participar en la Beta", "?participar=1", type="primary", width="stretch")
    ...
    if enviar:
        if not nombre.strip() or not email.strip() or not consentimiento:
            ...
            if ok:
                ...
                st.caption(f"ℹ️ Diagnóstico listo. {msg}")

elif menu == "🚀 Participar en la Beta":
    st.title("🚀 Participa en Control Pyme Beta")
    st.caption("Buscamos un grupo pequeño de Pymes para probar el producto, conversar con nosotros y ayudarnos a priorizar las funciones que realmente generan valor.")
    st.success("Los participantes Beta tendrán acceso anticipado y condiciones preferentes de lanzamiento.")
    with st.form("beta_interes"):
        c1, c2 = st.columns(2)
        beta_nombre = c1.text_input("Nombre *")
        beta_email = c2.text_input("Correo *")
        c3, c4 = st.columns(2)
        beta_telefono = c3.text_input("WhatsApp")
        beta_empresa = c4.text_input("Empresa")
        beta_plan = st.selectbox("¿Qué versión te interesaría probar?", ["Aún no sé", "Control", "Control Pro", "Control 360 + Legal"])
        beta_motivo = st.text_area("¿Qué problema te gustaría resolver primero con Control Pyme?")
    ...
    if beta_send:
        if len(beta_nombre.strip()) < 2 or "@" not in beta_email or not beta_consent:
            st.error("Completa nombre, correo válido y consentimiento.")
        else:
            ok, msg = save_beta_interest({
                "nombre": beta_nombre.strip(),
                "email": beta_email.strip(),
                "telefono": beta_telefono.strip(),
                "empresa": beta_empresa.strip(),
                "motivo": beta_motivo.strip(),
                "plan_interes": beta_plan,
                "consentimiento": True,
                "origen": "cta_beta",
            })
            if ok:
                st.success("✅ ¡Listo! Ya estás en la lista de Control Pyme Beta. Te contactaremos para coordinar el acceso.")
            else:
                st.error(msg)
