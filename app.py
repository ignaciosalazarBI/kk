...
# Conserva el origen aunque el visitante navegue por distintas secciones.
...
def funnel_event(evento: str) -> None:
    ...
    # Los eventos son auxiliares: si fallan, nunca bloquean la experiencia del visitante.
...
def save_lead(payload: dict) -> tuple[bool, str]:
    payload = {
        **payload,
        **TRACKING,
        "origen": "streamlit_beta",
        "estado": "nuevo",
    }
    return supabase_insert("leads", payload)
...
def save_beta_interest(payload: dict) -> tuple[bool, str]:
    ...
    return supabase_insert("beta_interes", payload)
...
def save_feedback(payload: dict) -> tuple[bool, str]:
    payload = {
        **payload,
        **TRACKING,
        "origen": "feedback_beta",
    }
    return supabase_insert("feedback_beta", payload)
...
default_index = default_map.get(start, 0)
menu = st.sidebar.radio("Control Pyme", menu_options, index=default_index)
...
if menu == "🏠 Inicio":
    st.markdown(
        '<div class="hero"><h2 style="margin:0">Control Pyme: entiende tu negocio en segundos</h2>'
        '<div style="color:#667085;margin-top:6px">Caja, cobranza, rentabilidad y alertas en un solo lugar, sin vivir en planillas.</div></div>',
        unsafe_allow_html=True,
    )
    ...
    cols = st.columns(5)
    with cols[0]:
        kpi("Ventas mes", money(KPIS["ventas"]), "Ejemplo neto")
    with cols[1]:
        kpi("Margen bruto", f'{KPIS["margen_pct"]:.1%}'.replace(".", ","), money(KPIS["margen"]))
    with cols[2]:
        kpi("Caja disponible", money(KPIS["caja"]), "Ejemplo crítico")
    with cols[3]:
        kpi("Por cobrar", money(KPIS["por_cobrar"]), "3 documentos ficticios")
    with cols[4]:
        kpi("Costo ventas", money(KPIS["costo_ventas"]), "Mes ficticio")

    st.subheader("🚨 Requiere acción")
    vencido = ventas[ventas.vence < TODAY]
    st.markdown(
        f'<div class="action"><b>Caja bajo mínimo.</b> Tienes {money(vencido.total.sum())} vencidos. '
        'Prioriza cobranza antes de pagos no críticos.</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="action"><b>{len(vencido)} clientes con documentos vencidos.</b> '
        f'El más antiguo lleva {(TODAY - vencido.vence.min()).days} días.</div>',
        unsafe_allow_html=True,
    )
    bajo = ventas[ventas.margen < 0.35]
    if not bajo.empty:
        st.markdown(
            f'<div class="action"><b>Margen bajo.</b> Revisa {bajo.iloc[0].cliente}: '
            f'margen estimado {bajo.iloc[0].margen:.0%}.</div>',
            unsafe_allow_html=True,
        )
    ...
    a, b, c = st.columns(3)
    with a:
        kpi("Cobros esperados", money(cobros_30), "Incluye vencidos ficticios")
    with b:
        kpi("Pagos esperados", money(pagos_30), "30 días")
    with c:
        kpi("Caja proyectada 30d", money(caja_30), "Escenario de ejemplo")
    ...
    fig = px.line(pd.DataFrame(movs), x="Fecha", y="Saldo proyectado", markers=True)
    st.plotly_chart(fig, width="stretch")

    st.markdown(
        '<div class="cta"><h3>🚀 ¿Te gustaría probar Control Pyme con tu negocio?</h3>'
        '<p>Únete al grupo Beta. No necesitas entregar información financiera real en esta etapa.</p></div>',
        unsafe_allow_html=True,
    )
    ...
    if enviar:
        if len(nombre.strip()) < 2:
            ...
            payload = {
                ...
                "email": clean(email.lower(), 180),
                "telefono": clean(whatsapp, 60),
                "empresa": clean(empresa, 180),
                "rubro": rubro,
                "trabajadores": trabajadores,
                "ventas_mensuales": ventas_mes,
                "herramienta_actual": clean(", ".join(sistema), 180),
                "dolor_principal": dolor,
                "vende_credito": credito,
                "interes_legal": legal,
                "disposicion_pago": precio,
                "comentario": clean(abierto, 1200),
                "consentimiento": True,
            }
    ...
    with st.form("beta_interes"):
        c1, c2 = st.columns(2)
        beta_nombre = c1.text_input("Nombre *", max_chars=120)
        beta_email = c2.text_input("Correo *", max_chars=180)
        c3, c4 = st.columns(2)
        beta_telefono = c3.text_input("WhatsApp", max_chars=60)
        beta_empresa = c4.text_input("Empresa", max_chars=180)
    ...
    if beta_send:
        if len(beta_nombre.strip()) < 2:
            st.error("Escribe tu nombre para continuar.")
        elif not valid_email(beta_email):
            st.error("Ingresa un correo válido.")
        elif not beta_consent:
            st.error("Necesitamos tu consentimiento para registrarte en la Beta.")
        else:
            ok, msg = save_beta_interest(
                {
                    "nombre": clean(beta_nombre, 120),
                    "email": clean(beta_email.lower(), 180),
                    "telefono": clean(beta_telefono, 60),
                    "empresa": clean(beta_empresa, 180),
                    "motivo": clean(beta_motivo, 1200),
                    "plan_interes": beta_plan,
                    "consentimiento": True,
                }
            )
            if ok:
    ...
    st.caption("Escenario completamente ficticio para mostrar cómo funcionará el módulo.")
    st.metric("Caja hoy", money(KPIS["caja"]))
    st.warning("Una Pyme puede tener utilidad y aun así quedarse sin caja. Control Pyme prioriza liquidez y fechas de cobro/pago.")
    tabla = pd.concat(
        [
    ...
    st.link_button("⭐ ¿Te serviría este módulo? Danos feedback", "?start=feedback", width="stretch")

elif menu == "🚨 Alertas":
    st.title("🚨 Alertas y cobranza")
    st.caption("Clientes y montos ficticios de demostración.")
    x = ventas.copy()
    ...
    st.title("📑 Legal Pyme")
    st.caption("Vista conceptual de una futura capa preventiva para contratos y obligaciones.")
    ...
    st.subheader("Ejemplo ficticio de análisis")
    c1, c2, c3 = st.columns(3)
    c1.metric("Renovación automática", "Sí", "Avisar 60 días antes")
    c2.metric("Riesgo", "Medio", "1 cláusula a revisar")
    c3.metric("Próximo hito", "15 oct", "Fecha ficticia")
    st.markdown(
        "**Lo que buscará la versión completa:** partes, duración, renovación, multas, obligaciones, fechas críticas, reajustes, garantías y un semáforo de riesgo."
    )
    st.info("Legal Pyme será una herramienta de apoyo y detección de riesgos; los casos relevantes deberán poder escalarse a un abogado.")
    st.link_button("⭐ Opinar sobre Legal Pyme", "?start=feedback", width="stretch")

elif menu == "💳 Planes Beta":
    st.title("💳 Planes Beta")
    st.caption("Precios de referencia que estamos validando. Todavía no existe cobro dentro de esta Beta pública.")
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    st.title("⭐ Ayúdanos a mejorar Control Pyme")
    st.caption("Toma menos de un minuto. Este feedback es una de las partes más importantes de la Beta.")

    with st.form("feedback_beta"):
        puntuacion = st.slider("En general, ¿qué nota le pondrías a esta Beta?", min_value=1, max_value=5, value=4)
    ...
    if feedback_send:
        if feedback_email.strip() and not valid_email(feedback_email):
            st.error("El correo opcional no tiene un formato válido.")
        elif not feedback_consent:
            st.error("Necesitamos tu consentimiento para guardar el feedback.")
        else:
            ok, msg = save_feedback(
                {
                    "puntuacion": int(puntuacion),
                    "utilidad": utilidad,
                    "parte_util": clean(parte_util, 120),
                    "comentario": clean(comentario, 1200),
                    "email": clean(feedback_email.lower(), 180),
                    "consentimiento": True,
                }
            )
            if ok:
                funnel_event("feedback_enviado")
                st.success("🙌 Gracias. Tu opinión quedó registrada y nos ayuda a decidir qué construir primero.")
            else:
                st.error(msg)

else:
    st.title("🔒 Privacidad de la Beta")
    st.caption("Versión simple y transparente para esta etapa de validación.")
    st.markdown(
        """
