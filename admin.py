...
def fetch_leads() -> tuple[pd.DataFrame | None, str]:
    ...
    if not url or not key or not admin_token:
...
legal_count = leads.get("interes_legal", pd.Series(dtype=str)).fillna("").astype(str).str.lower().isin(["sí", "si", "true", "tal vez"]).sum()
would_pay = ~leads.get("disposicion_pago", pd.Series([""] * n)).fillna("").astype(str).str.contains("Aún no pagaría", case=False, regex=False)
...
if "dolor_principal" in leads.columns:
    dolor = leads["dolor_principal"].fillna("Sin respuesta").value_counts().rename_axis("Problema").reset_index(name="Leads")
...
with c5:
    ...
    if "disposicion_pago" in leads.columns:
        pago = leads["disposicion_pago"].fillna("Sin respuesta").value_counts().rename_axis("Rango").reset_index(name="Leads")
        st.dataframe(pago, hide_index=True, width="stretch")
with c6:
    st.subheader("Rubros")
    if "rubro" in leads.columns:
        rubros = leads["rubro"].fillna("Sin respuesta").value_counts().rename_axis("Rubro").reset_index(name="Leads")
...
if "created_at" in view.columns:
    view["created_at"] = view["created_at"].dt.tz_convert("America/Santiago").dt.strftime("%d-%m-%Y %H:%M")
