...
if not leads.empty:
    frames.append(pd.DataFrame({
        "_id": leads["id"], "_entity": "lead", "Fecha": leads["created_at"], "Origen": "Diagnóstico",
        "Nombre": leads.get("nombre", ""), "Empresa": leads.get("empresa", ""), "Email": leads.get("email", ""),
        "Necesidad": leads.get("dolor_principal", ""), "Estado": leads.get("estado", "nuevo"),
        "Canal": leads.get("utm_source", "direct"),
    }))
if not beta.empty:
    frames.append(pd.DataFrame({
        "_id": beta["id"], "_entity": "beta", "Fecha": beta["created_at"], "Origen": "Interés Beta",
        "Nombre": beta.get("nombre", ""), "Empresa": beta.get("empresa", ""), "Email": beta.get("email", ""),
        "Necesidad": beta.get("motivo", ""), "Estado": beta.get("estado", "nuevo"),
...
visitas = primeras = diag_vistos = intereses = feedbacks = 0
if not eventos.empty:
    visitas = eventos.loc[eventos.evento == "visita", "session_id"].nunique()
    primeras = eventos.loc[eventos.evento == "primera_interaccion", "session_id"].nunique()
...
m2.metric("Primer clic", primeras, f"{primeras/visitas*100:.0f}%" if visitas else "—")
m3.metric("Diagnósticos", diag_vistos, f"{diag_vistos/visitas*100:.0f}%" if visitas else "—")
m4.metric("Interés Beta", intereses, f"{intereses/visitas*100:.0f}%" if visitas else "—")
m5.metric("Feedback", feedbacks, f"{feedbacks/visitas*100:.0f}%" if visitas else "—")
m6.metric("Contactos", unique_contacts)

T1, T2, T3, T4, T5 = st.tabs(["📈 Embudo", "🧩 Módulos", "🧭 Comportamiento", "👥 Contactos", "⭐ Feedback"])

with T1:
    funnel = pd.DataFrame({
        "Etapa": ["Visitas", "Primer clic", "Diagnóstico", "Interés Beta", "Feedback"],
        "Personas": [visitas, primeras, diag_vistos, intereses, feedbacks],
    })
    funnel["% sobre visitas"] = [100 if visitas else 0] + [(x / visitas * 100 if visitas else 0) for x in [primeras, diag_vistos, intereses, feedbacks]]
    ...
    if not eventos.empty:
        ...
        out = pd.DataFrame({
            "Canal": canal.index,
            "Visitas": canal.get("visita", 0),
            "Primer clic": canal.get("primera_interaccion", 0),
            "Diagnóstico": canal.get("diagnostico_visto", 0),
            "Interés Beta": canal.get("beta_interes", 0),
            "Feedback": canal.get("feedback_enviado", 0),
...
with T2:
    ...
    if eventos.empty:
        ...
        mod = eventos[
            eventos.evento.str.startswith("modulo_", na=False)
            & ~eventos.evento.str.startswith("modulo_prioridad_", na=False)
            & (eventos.evento != "modulos_interes_guardados")
        ].copy()
        if mod.empty:
            st.info("Todavía no hay selecciones de módulos con la nueva versión.")
        else:
        ...
        if pri.empty:
            st.info("Aún no hay prioridades explícitas.")
        else:
            ...
            st.bar_chart(ptable)
            st.dataframe(ptable.reset_index(), hide_index=True, width="stretch")
...
with T3:
    ...
    if eventos.empty:
        ...
        if screen.empty:
            st.info("Aún no hay suficiente tráfico con tracking por pantalla.")
        else:
...
with T4:
    ...
    if contacts.empty:
        ...
        edited = st.data_editor(
            ...
            hide_index=True,
            width="stretch",
            disabled=["_id", "_entity", "Fecha", "Origen", "Nombre", "Empresa", "Email", "Necesidad", "Canal"],
            column_config={
                "_id": None,
                "_entity": None,
                "Estado": st.column_config.SelectboxColumn("Estado", options=["nuevo", "contactado", "beta", "cliente", "descartado"], required=True),
            },
            key="contact_editor",
        )
...
with T5:
    if feedback.empty:
        st.info("Todavía no hay feedback completo.")
    else:
...
st.divider()
st.caption("Panel privado · datos vía Edge Function protegida")
